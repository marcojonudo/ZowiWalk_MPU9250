...
void Zowi::_moveServos(int time, int servo_target[]) {
	...
	if (time > 10) {
		for (int i = 0; i < 4; i++) {
			increment[i] = ((servo_target[i]) - servo_position[i])
					/ (time / 10.0);
		}
		...
		for (int iteration = 1; millis() < final_time; iteration++) {
			...
			for (int i = 0; i < 4; i++) {
				servo[i].SetPosition(
						servo_position[i] + (iteration * increment[i]));
			}
...
void Zowi::up(float steps, int T) {
	int up[] = { 90, 90, 150, 30 };
	_moveServos(T, up);
}

void Zowi::down(float steps, int T) {
	int down[] = { 90, 90, 90, 90 };
	_moveServos(T, down);
}

void Zowi::moveFeet(int leftFoot, int rightFoot) {
	int moveFeet[] = { 90, 90, leftFoot, rightFoot };
	_moveServos(1000, moveFeet);
}

void Zowi::moveHip(int leftHip, int rightHip) {
	int moveHip[] = { leftHip, rightHip, 90, 90 };
	_moveServos(1000, moveHip);
}
...
void Zowi::turnWithoutMoving(int T, int dir) {

	//Parameters of all the movements. Default: Left bend
	int bend1[4] = { 90, 90, 60, 35 };
	int bend2[4] = { 90, 90, 60, 90 };

	//Parameters for turning hip while foot in the air
	int hip[4] = { 115, 90, 60, 90 };
	...
	_moveServos(T2, bend1);
	_moveServos(T2, bend2);
	_moveServos(T2, hip);
	_moveServos(T2, turn);
	delay(T * 2);
	//_moveServos(T2/2, turnFoot);
...
void Zowi::ascendingTurn(float steps, int T, int h) {
	...
	int A[4] = { h, h, h, h };
	int O[4] = { 0, 0, h + 4, -h + 4 };
	double phase_diff[4] = { DEG2RAD(-90), DEG2RAD(90), DEG2RAD(-90), DEG2RAD(
			90) };
...
void Zowi::flapping(float steps, int T, int h, int dir) {
	...
	double phase_diff[4] = { DEG2RAD(0), DEG2RAD(180), DEG2RAD(-90 * dir),
			DEG2RAD(90 * dir) };
...
unsigned long int Zowi::getMouthShape(int number) {
	unsigned long int types[] = { zero_code, one_code, two_code, three_code,
			four_code, five_code, six_code, seven_code, eight_code,
			nine_code, smile_code, happyOpen_code, happyClosed_code, heart_code,
			bigSurprise_code, smallSurprise_code, tongueOut_code,
			vamp1_code, vamp2_code, lineMouth_code, confused_code,
			diagonal_code, sad_code, sadOpen_code, sadClosed_code,
			okMouth_code, xMouth_code, interrogation_code, thunder_code,
			culito_code, angry_code, al_code };
...
unsigned long int Zowi::getAnimShape(int anim, int index) {
	...
	unsigned long int littleUuh_code[] = { 0b00000000000000001100001100000000,
	...
	unsigned long int dreamMouth_code[] = { 0b00000000000000000000110000110000,
			0b00000000000000010000101000010000,
			0b00000000011000100100100100011000,
			0b00000000000000010000101000010000 };

	unsigned long int adivinawi_code[] = { 0b00100001000000000000000000100001,
	...
	unsigned long int wave_code[] = { 0b00001100010010100001000000000000,
...
void Zowi::bendTones(float initFrequency, float finalFrequency, float prop,
